# Load src/words_alpha.json

import json
import os

def load_words():
	with open(os.path.join(os.path.dirname(__file__), 'src/words_alpha.json')) as word_file:
...
# Save it to a new json at src/words_alpha_filtered.json
def save_words(words, ending):
	with open(os.path.join(os.path.dirname(__file__), f'src/words_alpha_filtered_{ending}.json'), 'w') as outfile:
